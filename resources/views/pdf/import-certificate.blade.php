...
<body>
    ...
    <div class="section vehicle-info">
        ...
        <table>
            ...
            <tr>
                ...
                <td>{{ $import->plate_original ?? 'Pendiente' }}</td>
            </tr>
            <tr>
                <td class="label">Matrícula Española (ES):</td>
                <td class="plate">{{ $plate_new }}</td>
            </tr>
            <tr>
                <td class="label">País Origen:</td>
                <td>{{ $import->origin_country }}</td>
            </tr>
        </table>
    </div>
    ...
    <div class="section valuation">
        ...
        <table>
            <tr>
                ...
                <td>{{ number_format($valuation['valuation']->estimated_value, 2) }}€</td>
            </tr>
            <tr>
                <td>IEDMT (impuesto CO₂):</td>
                <td>{{ number_format($valuation['taxes']['iedmt'], 2) }}€</td>
            </tr>
            <tr>
                <td>ITP estimado:</td>
                <td>{{ number_format($valuation['taxes']['itp_estimate'], 2) }}€</td>
            </tr>
            <tr>
                <td class="total">Coste total estimado:</td>
                <td class="total">{{ number_format($valuation['total_cost'], 2) }}€</td>
            </tr>
        </table>
    </div>
    ...
    <div class="section">
        <h2>Información</h2>
        <p><strong>Fecha certificado:</strong> {{ now()->format('d/m/Y') }}</p>
        <p><strong>Estado importación:</strong> {{ $import->current_step->getLabel() }}</p>
